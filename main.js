...
async function getEmail(request) {
    ...
    await lostItemPromise;
    await foundItemPromise;
    if(request.user.id === lostItem.get(KEY_LOST_BY).id) {
        const res = await new Parse.Query("User").equalTo("objectId", foundItem.get(KEY_FOUND_BY).id).find({ useMasterKey: true });
        if(res){
            const user = res[0];
            console.log("Email is: " + user.get("email"));
            return user.get("email");
        }
    } else if(request.user.id === foundItem.get(KEY_FOUND_BY).id) {
        const res = await new Parse.Query("User").equalTo("objectId", lostItem.get(KEY_LOST_BY).id).find({ useMasterKey: true });
        if(res){
            const user = res[0];
            console.log("Email is: " + user.get("email"));
            return user.get("email");
